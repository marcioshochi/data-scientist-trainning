...
spark = SparkSession.builder.master("local").appName("classify_solutions").getOrCreate()
...
# **Note:** The `ave(high_rating)` gives the observed fraction of a high ratings.
...
# **Note:** `vehicle_color_cd` is stored as a `SparseVector`.
...
features = ["reviewed", "vehicle_year", "vehicle_color_cd", "vehicle_noir"]
assembler = VectorAssembler(inputCols=features, outputCol="features")
